...
if (!MONGO) {
  console.error(
    "MongoDB connection string (MONGO) is missing in environment variables"
  );
...
app.post("/tasks", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ error: "Error creating task", details: err.message });
  }
});
  ...
  try {
    ...
    if (!project) {
      return res
        ...
        .send({
          error:
            "Project not found or you do not have permission to rename this project",
        });
    }
